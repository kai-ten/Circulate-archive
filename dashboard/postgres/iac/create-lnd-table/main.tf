...
data "aws_iam_policy_document" "create_lnd_table_policy" {
...
module "circulate_create_lnd_table" {
  source          = "../../../../integrations/modules/go-lambda"
  name            = "${var.name}-${var.env}"
  lambda_name     = "${var.name}-${var.env}-${var.service}"
  src_path        = "../../lib/create-lnd-table"
  iam_policy_json = data.aws_iam_policy_document.create_lnd_table_policy.json
  timeout = 30
  vpc_config = {
...
resource "null_resource" "db_setup" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
			...
			EOF
    environment = {
      FUNCTION_NAME     = module.circulate_create_lnd_table.lambda_function.function_name
    }
    interpreter = ["bash", "-c"]
  }
  depends_on = [
